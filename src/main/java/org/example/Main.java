...
public class Main {
	...
	private static void printDiff(String donation_name, String first_person, String second_person) {
		...
		String donated_less = Objects.equals(donated_more, first_person) ? second_person : first_person;
		if (donated_more != null) {
			System.out.printf("%s is donated more then %s\n", donated_more, donated_less);
		} else {
			donated_more = Objects.equals(donated_less, first_person) ? second_person : first_person;
	...
	private static void load_charity(String charity_name) {
		try {
			...
			System.out.println("[ERROR] Charity not found! Getting fallback charity...");
